    ...
    for (let qIdx = 0; qIdx < (r.questions ?? []).length; qIdx++) {
      ...
      if (q.id) {
        await prisma.question.update({
          where: { id: q.id },
          data: { text: q.text, purpose: q.purpose ?? null, metaTag: q.metaTag ?? null, orderIndex: qIdx, isArchived: false },
        });
      } else {
        await prisma.question.create({
          data: { roundId: round.id, text: q.text, purpose: q.purpose ?? null, metaTag: q.metaTag ?? null, orderIndex: qIdx },
        });
      }
    }
  }
