        ...
        <div className="mb-8">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
            <div className="bg-[#161b22] border border-[#21262d] rounded-xl p-5">
              <div className="text-3xl font-bold text-white">{dash.totalInterviews}</div>
              <div className="text-sm text-[#8b949e] mt-1">Total Interviews</div>
            </div>
          </div>

          {dash.stats.filter((s) => s.totalInterviews > 0).length > 0 && (
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-[#21262d]">
              ...
              <th
                className="text-left px-5 py-3 text-[#8b949e] font-medium cursor-pointer hover:text-white"
          ...
          <tbody className="divide-y divide-[#21262d]">
            ...
            {filtered.map((iv) => {
              ...
              return (
                ...
                >
                  <td className="px-5 py-3 text-white font-medium">{iv.candidate.name}</td>
                  <td className="px-5 py-3 text-[#8b949e]">{iv.role.name}</td>
                  ...
                  <td className="px-5 py-3 text-[#8b949e] text-xs">{rec?.label ?? "—"}</td>
                  <td className="px-5 py-3">
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${iv.status === "completed" ? "bg-emerald-900/50 text-emerald-300 border border-emerald-800" : "bg-blue-900/50 text-blue-300 border border-blue-800"}`}>
